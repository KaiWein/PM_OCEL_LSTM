...
# helper variables
lastcase = '' #discribes last case 
line = ''
firstLine = True # boolean if line is first line
Ptimes = []
Ptimes2 = []
nb_items = []
...
ascii_offset = 64
# here some values are callculate out of the eventlog time look above for them
for row in spamreader:  # the rows are "CaseID,ActivityID,CompleteTimestamp,related_objects"
    ...
    Ptimediff = 86400 * Ptimesincelastevent.days + Ptimesincelastevent.seconds # convert differance into seconds 1day=86400 sec
...
PtimeseqsF = [] # time sequences (differences between last activitie and currant activity = rmaining time)
    ...
    for t, char in enumerate(sentence):
        ...
        for c in chars:
            if c == char:  # this will encode present events to the right places
                X[i, t + leftpad, char_indices[c]] = 1                  # activitiy of event
        X[i, t + leftpad, len(chars)] = t + 1                           # number of case +1 
        X[i, t + leftpad, len(chars) + 1] = sentence_t[t] / divisor     # time between events divided by average time between events
        X[i, t + leftpad, len(chars) + 2] = sentence_t2[t] / divisor2   # time since forst of case divided by average time between first an current
        X[i, t + leftpad, len(chars) + 3] = sentence_t3[t] / 86400      # time after midnight in seconds dividet by seconds of day = procent of day
        X[i, t + leftpad, len(chars) + 4] = sentence_t4[t] / 7          # weekday divdet by 7 = procent od week
        X[i, t + leftpad, len(chars) + 5] = sentence_t5[t]              # number of objects

    for c in target_chars:  # 
        if c == next_chars[i]:
            y_a[i, target_char_indices[c]] = 1 - softness               # next char for prediction
        else:
            y_a[i, target_char_indices[c]] = softness / (len(target_chars) - 1)
    y_t[i] = next_t / divisor                                           # time betwwen events divided by average    
    y_tr[i] = next_tr / divisorTR                                       # remaining time divided by avarage
