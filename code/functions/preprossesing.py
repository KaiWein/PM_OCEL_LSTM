...
def gen_features(OCEL,add_last_case=True, columns_to_encode = ['Activity', 'Customers']):
    ...
    if ('!' not in list(OCEL['Activity'].unique())) and add_last_case:
        ...
        OCEL = pd.concat([OCEL, new_rows], ignore_index=True)
...
def gen_traces_and_maxlength_of_trace(OCEL):
    ...
    maxlen = max([len(x) for x in activity_sequences['Activity']])
